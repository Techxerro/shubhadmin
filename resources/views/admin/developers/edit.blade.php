...
<div class="max-w-4xl">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit Developer</h1>
        <p class="text-sm text-gray-500">Update developer details.</p>
    </div>

    <div class="rounded-2xl bg-white p-6 shadow-sm border border-gray-100">
        <form action="{{ route('admin.developers.update', $developer->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea
                    ...
                    rows="5"
                    class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                >{{ old('description', $developer->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
